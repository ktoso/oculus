...
class YouTubeMetaDeserializerTest extends FlatSpec with ShouldMatchers {

  it should "deserialize youtube metadata file" in {
    // given
    val json =
      """
        |{
    ...
    // when
    val meta = YouTubeMeta.from(json)

    // then
    meta.upload_date should equal ("20101223")
  }
