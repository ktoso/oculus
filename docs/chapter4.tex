...
% As this thesis is not focused on development of image analysis algorithms, the descriptions will focus on the distributed and big-data parts of the problem. While assuming that the used image correlation algorithm (\textit{phash} -- see Section \ref{sec:phash}) performs well enough for the job at hand. Since even when using phash the problem of finding similar images is inherently $O(n^2)$ complex, the primary focus of the methods described bellow is  ...... [wanted to explain that we lower the N, but it's still n^2 by the end of the day...]
